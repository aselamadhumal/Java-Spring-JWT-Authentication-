...
public class MaskingPatternLayout extends PatternLayout {
    ...
    private String maskMessage(String message) {
        ...
        // Use a loop to find and mask all occurrences of the pattern
        while (matcher.find()) {
            // Iterate over each matched group in the pattern
            for (int group = 1; group <= matcher.groupCount(); group++) {
                if (matcher.group(group) != null) {
                    // Replace the matched group with a fixed-length mask (e.g., "*****")
                    IntStream.range(matcher.start(group), matcher.end(group))
                            .forEach(i -> sb.setCharAt(i, '*'));

                    sb.replace(matcher.start(group),matcher.end(group), "*****"); // Replace the entire group with "*****"

                    break; // Exit the loop to avoid overlapping matches
